...
export default function PatientDetail({ patient, onBack }) {
  ...
  useEffect(() => {
    // Si no hay paciente, no cargar datos adicionales
    if (!patient) {
      console.log("🚫 PatientDetail: No hay paciente, cancelando carga")
      setLoading(false)
      return
    }
  ...
  // Si no hay paciente después de la carga
  if (!patient) {
    return (
      <div className="space-y-6">
        <Button onClick={onBack} variant="ghost" className="flex items-center space-x-2">
              ...
              <div className="space-y-3">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{eps ? eps.nombreEPS : "No afiliado a EPS"}</p>
                    {patient.epsId && !eps && <p className="text-xs text-yellow-600">(ID: {patient.epsId})</p>}
                  </div>
                </div>
                <div>
            ...
            <div>
              <div className="space-y-3">
                  ...
                  <p className="font-medium">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        patient.estadoAfiliacion ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                      }`}
                    >
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {patient.fechaAfiliacion
                      ? new Date(patient.fechaAfiliacion).toLocaleDateString()
                      : "No especificada"}
                  </p>
                </div>
              </div>
